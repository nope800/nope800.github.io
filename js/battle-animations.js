...
var isBottom=!targetsSelf&&this.battle.dex.moves.get(moveid).category==='Bottom';
...
var $st=pokemon.sprite.$statbar.find('div.st');
var w=pokemon.hpWidth(150);
var hpcolor=BattleScene.getHPColor(pokemon);
var callback;
if(hpcolor==='y'){
callback=function(){$st.addClass('st-yellow');};
}
if(hpcolor==='r'){
callback=function(){$st.addClass('st-yellow st-red');};
}
...
var callback;
if(hpcolor==='g'){
callback=function(){$st.removeClass('st-yellow st-red');};
}
if(hpcolor==='y'){
callback=function(){$st.removeClass('st-red');};
}
...
var ratio=pokemon.st/pokemon.maxhp;
...
var w=pokemon.hpWidth(150);
var $st=this.$statbar.find('.st');
$st.css({
width:w,
'border-right-width':w?1:0
});
if(hpcolor==='g')$st.removeClass('st-yellow st-red');else
if(hpcolor==='y')$st.removeClass('st-red').addClass('st-yellow');else
$st.addClass('st-yellow st-red');
...
}else if(pokemon.status==='heat'){
status+='<span class="heat">HEAT</span> ';
}else if(pokemon.status==='trance'){
status+='<span class="trance">TRANCE</span> ';
}else if(pokemon.status==='embarrassed'){
...
status+='<img src="'+Dex.resourcePrefix+'sprites/types/'+encodeURIComponent(type)+'.png" alt="'+type+'" class="pixelated" width="16" height="16" /> ';
}
}
if(pokemon.volatiles.typeadd){
var _type=pokemon.volatiles.typeadd[1];
status+='+<img src="'+Dex.resourcePrefix+'sprites/types/'+_type+'.png" alt="'+_type+'" class="pixelated" width="16" height="16" /> ';
}
...
if(_i35==='roost'&&!pokemon.getTypeList().includes('Vanilla'))continue;
...
$hptext.hide();
$hptextborder.hide();
}else if(this.scene.battle.hardcoreMode||this.scene.battle.reportExactSt){
